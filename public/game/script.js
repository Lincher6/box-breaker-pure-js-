...
const selectors = [
    ...
    'cancel', 'total', 'results', 'username', 'hi-score', 'user-card'
];

const [
    $field, $start, $timer, $score, $pause, $dialog,
    $cancel, $total, $results, $username, $hiScore, $userCard
...
let user = JSON.parse($userCard.dataset.user);
...
async function saveResult(event) {
    ...
    const { topResults, hiScore } = await response.json();
